...
### 데이터셋 로딩 시간 (nvidia/OpenCodeGeneticInstruct, 750만 샘플)
...
| **총 로딩 시간** | **~8분** | **~2-3분** | **~1.5-2분** ⚡ | **75-80% 감소** |
...
### 토크나이징 시간 (750만 샘플) ⚡ v3.2 NEW!

| 설정 | 이전 | v3.2 ⚡ | 개선율 |
|------|------|---------|--------|
| **tokenize_num_proc** | **4** | **16** ⚡ | **4x** |
| **tokenize_batch_size** | **1000** | **5000** ⚡ | **5x** |
| **tokenize_writer_batch** | **-** | **20000** ⚡ | **I/O 2x** |
| **keep_in_memory** | **True** | **False** ⚡ | **메모리 절약** |
| **총 토크나이징 시간** | **~43분** | **~7-8분** ⚡ | **5-6배 빠름!** |

**v3.2 토크나이징 개선 포인트** ⭐:
- ⚡ 병렬 처리: num_proc=4 → 16 (4배)
- ⚡ 배치 크기: 1000 → 5000 (5배)
- ⚡ I/O 최적화: writer_batch_size=20000 (2배)
- ⚡ 메모리 맵: keep_in_memory=False (메모리 절약)

### 총 처리 시간

| 단계 | 이전 | v3.2 ⚡ | 개선 |
|------|------|---------|------|
| 데이터셋 로딩 | ~8분 | ~1.5-2분 | 75-80% |
| 토크나이징 | ~43분 | ~7-8분 | 81-84% |
| **총 시간** | **~51분** | **~9-10분** | **⚡ 80%+ 단축!** |
